...
起動時に **部署選択 → ロール選択** の順で表示される（ともに `sessionStorage` に保存）。

- **部署**: `GET /api/departments` の一覧から選択（弁当惣菜部／餅部／漬物部）。選択した部署のデータのみを表示・編集する。
  - **メンバー**は選んだ部署に固定。**管理者（本部）**はサイドバーの部署スイッチャーで全部署を切替可能。
  - 部署の `config.features` に応じてメニューを出し分け（餅部・漬物部では「発注」「週間献立」を非表示＝コア機能のみ）。
  - ※ 現状は「管理者ロール＝本部（全部署切替可）」として扱う。部署ごとの管理者と本部管理者を分けたい場合は、部署別PIN等での拡張が必要。
